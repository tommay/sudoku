...
class Puzzle
  ...
  def solve
    ...
    when next_position.possible.empty?
      ...
      next_position.possible.map do |digit|
        ...
        [puzzle, puzzle.future.solve]
      end.flat_map do |puzzle, future|
        future.value.tap {puzzle.terminate}
      end
    end
  end
...
if __FILE__ == $0
  puzzle = Puzzle.new(filename: ARGV[0])
  puts "#{puzzle.solve.size} solutions"
  puzzle.terminate
end
